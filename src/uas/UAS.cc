...
void UAS::setExternalControlSetpoint6DOF(float roll, float pitch, float yaw, float thrust, quint16 buttons, int joystickMode, float lat , float forward, float gain)
{
    ...
    static float manualYawAngle = 0.0;
    static float manualThrust = 0.0;
    static float manualLat = 0.0;
    static float manualForward = 0.0;
    static quint16 manualButtons = 0;
    static quint8 countSinceLastTransmission = 0; // Track how many calls to this function have occurred since the last MAVLink transmission
    ...
    if (countSinceLastTransmission++ >= 5) {
        ...
        countSinceLastTransmission = 0;
             ...
             (!qIsNaN(yaw) && yaw != manualYawAngle) || (!qIsNaN(thrust) && thrust != manualThrust) ||
               (!qIsNaN(lat) && thrust != manualLat) || (!qIsNaN(thrust) && forward != manualForward) ||
             buttons != manualButtons) {
        sendCommand = true;
    ...
    if (sendCommand) {
        ...
        //not not supported for 6dof yet
        if (joystickMode == Vehicle::JoystickModeAttitude) {
           /* // send an external attitude setpoint command (rate control disabled)
            float attitudeQuaternion[4];
            mavlink_euler_to_quaternion(roll, pitch, yaw, attitudeQuaternion);
...
void UAS::setManual6DOFControlCommands(double x, double y, double z, double roll, double pitch, double yaw)
{
   ...
   // If system has manual inputs enabled and is armed
    ...
    {
        mavlink_message_t message;
        //why would you do this, the rotation would be treated as a torque vector independed to each other,
        //Treat them as a rate of change not absolutes....
        float q[4];
